...
import route from './Route76-fixed.json';
...
export default class Car {
  ...
  constructor({ randomPosition }) {
    ...
    const index = !randomPosition ? 0 : Math.round((Math.random() * route.length));
    this.dTime = route[index].time;
    this.type = Car.TYPE;
    this.lastRenderTime = Date.now() + this.dTime;
    this.id = ++Car.AMOUNT;
    this.lat = route[index].lat;
    this.lng = route[index].lng;
    this.affects = [];
  ...
  render(timestamp) {
    ...
    if (timestamp > _.last(route).time) {
      timestamp = timestamp % _.last(route).time;
    }
    ...
    if(this.affects.find(e => e.type === CarBreakDownEvent.EVENT_TYPE)) {
      ...
      newPoint = route.find(point => point.time > timestamp);
      if(!newPoint) {
        console.log('[ERROR] Not found new point for route', timestamp);
        return null;
      }
    ...
    if(!newPoint) {
      console.log('[ERROR] Not found new point for route');
      return null;
    }
